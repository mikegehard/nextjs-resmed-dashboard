...
# Test app tooling via npx (installed as devDependencies)
echo -n "Testing tsc (via npx)... "
if output=$(docker run --rm project-container:test npx tsc --version 2>&1); then
  echo "✓"
else
  exit_code=$?
  echo "✗ (exit code: $exit_code)"
...
echo -n "Testing eslint (via npx)... "
if output=$(docker run --rm project-container:test npx eslint --version 2>&1); then
  echo "✓"
else
  exit_code=$?
  echo "✗ (exit code: $exit_code)"
...
echo -n "Testing next build... "
if output=$(docker run --rm project-container:test npx next build 2>&1); then
  echo "✓"
else
  exit_code=$?
  echo "✗ (exit code: $exit_code)"
